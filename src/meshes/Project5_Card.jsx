...
import { FaceData, DetailsData, ProjMedia } from '../faces/Project5_Data'

export default function Project5_Card({isOpened, onProj5Clicked, Proj5Clicked, ...props}) {
  const group = useRef()
  ...
  const positionSpring = useSpring({
    ...
    pos: Proj5Clicked ? [20, -0.3, -0.93] : [-0.01, 0, -0.758],
    config: Proj5Clicked ? { mass: 1, tension: 60, friction: 8 } : { mass: 1, tension: 50, friction: 20 },
  });

  const rotationSpring = useSpring({
    ref: rotationSpringRef,
    rot: Proj5Clicked ? [0, THREE.MathUtils.degToRad(900), THREE.MathUtils.degToRad(2.3)] : [0, 0, 0],
  });

  useChain(Proj5Clicked ? [positionSpringRef, rotationSpringRef] : [rotationSpringRef, positionSpringRef], [0, secTime]);

  const helper = () => {
    if(isOpened) {
      console.log('clicked 5');
      if (Proj5Clicked) {              //time for pos to play after rot
        toggleSecTime(0.4);
                      ...
                      <primitive object={nodes.fourthfold}>
                        <primitive object={nodes.fifthsleeve}>
                          ...
                          <Html
                            occlude="blending"
                            wrapperClass="nodes"
                            position={[4.08, 2.93, 0]}
                            transform
                            distanceFactor={8.1}
                            rotation-y={THREE.MathUtils.degToRad(-0.7)}
                            rotation-x={THREE.MathUtils.degToRad(180)}
                            style={{ pointerEvents: 'none' }}
                          >
                            <Project_Details />
